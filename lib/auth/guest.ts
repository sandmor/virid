...
export async function createGuestSession(maxAgeSeconds = 60 * 60 * 24) { // default 24h
  const now = Date.now();
  const email = `guest-${now}`; // deterministic pattern used for UI detection
  const uid = generateUUID();
  const exp = now + maxAgeSeconds * 1000;
  const payload: GuestSessionPayload = { uid, email, type: "guest", iat: now, exp };
...
export async function readGuestSession(): Promise<GuestSessionPayload | null> {
  ...
  try {
    ...
    if (typeof parsed.exp === "number" && parsed.exp < Date.now()) {
      ...
      try { await clearGuestSession(); } catch {}
      return null;
    }
    return parsed;
  } catch {
