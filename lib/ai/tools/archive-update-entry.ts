...
const canonicalSchema = z.object({
  slug: z.string().min(1).describe("Identifier of the archive entry to update"),
  newEntity: z.string().min(1).max(512).optional().describe("New entity/title (<=512 chars)"),
  addTags: z.array(z.string().min(1)).max(64).optional().describe("Tags to add (duplicates ignored, normalized)"),
  removeTags: z.array(z.string().min(1)).max(64).optional().describe("Tags to remove"),
  body: z.string().optional().describe("Full replacement body (mutually exclusive with appendBody)"),
  appendBody: z.string().optional().describe("Text to append to existing body (mutually exclusive with body). Avoid passing 'false' or boolean-like strings."),
}).superRefine((val, ctx) => {
  if (val.body && val.appendBody) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Provide only one of body or appendBody" });
  }
  if (val.appendBody === "false") {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "appendBody should not be the literal string 'false'; omit the field instead" });
  }
});

// Extremely lenient outer schema: accept any object (the model can send natural variants)
const rawInputSchema = z.record(z.string(), z.any()).describe(
...
function extractSlug(raw: any): string | undefined {
  return pickFirst(raw.slug, raw.id, raw.entry, raw.file, raw.targetSlug, raw.identifier)?.toString();
}

function normalizeUpdateRaw(raw: any, currentTags: string[] | null): Canonical | { error: string; issues?: string[] } {
  ...
  else if (!newEntity && raw.rename && typeof raw.rename === "object" && typeof raw.rename.to === "string") newEntity = raw.rename.to;

  // Body replacement vs append
  let body = pickFirst(raw.body, raw.fullBody, raw.replaceBody, raw.content, raw.text, raw.replace);
  let appendBody = pickFirst(raw.appendBody, raw.append, raw.addition, raw.add, raw.extra);
  ...
  if (appendBody && ["false","none","null"].includes(String(appendBody).toLowerCase())) appendBody = undefined;
  if (body && ["false","none","null"].includes(String(body).toLowerCase())) body = undefined;

  // Tags additions / removals
  const addTags = coerceStringArray(pickFirst(raw.addTags, raw.addTag, raw.tagsAdd, raw.tagAdd));
  const removeTags = coerceStringArray(pickFirst(raw.removeTags, raw.removeTag, raw.tagsRemove, raw.tagRemove));
  ...
  if (setTags && setTags.length && currentTags) {
    ...
    return { slug, newEntity, addTags: mergedAdd.size ? Array.from(mergedAdd) : undefined, removeTags: mergedRemove.size ? Array.from(mergedRemove) : undefined, body, appendBody };
  }

  return { slug, newEntity, addTags: addTags && addTags.length ? addTags : undefined, removeTags: removeTags && removeTags.length ? removeTags : undefined, body, appendBody };
}
...
`,
    ...
    execute: async (rawInput) => {
      if (!session.user?.id) return { error: "No active user session", code: "unauthorized" } as const;

      // Attempt early slug extraction to fetch current tags (for setTags semantics)
      const provisionalSlug = extractSlug(rawInput);
      const userId = session.user.id;
      const current = provisionalSlug ? await getArchiveEntryBySlug({ userId, slug: provisionalSlug }) : null;
      if (provisionalSlug && !current) {
        // If the caller provided a slug-like field but it doesn't exist, short-circuit
        return { error: "Entry not found", code: "not_found", hints: ["Ensure the slug exists before updating"] } as const;
      }

      const normalized = normalizeUpdateRaw(rawInput, current ? current.tags : null);
      if ("error" in normalized) {
        return { error: normalized.error, code: "bad_request", issues: normalized.issues } as const;
      }
      ...
      let { slug, newEntity, addTags, removeTags, body, appendBody } = parsed.data;
      ...
      try {
        const updated = await updateArchiveEntry({ userId, slug, newEntity, addTags, removeTags, body, appendBody });
        if (!updated) {
          return { error: "Entry not found", code: "not_found" } as const;
        }
        const refetched = await getArchiveEntryBySlug({ userId, slug });
        if (!refetched) return { error: "Entry not found after update", code: "not_found" } as const;
        return {
          slug: refetched.slug,
          ...
          noOp: updated.updatedAt.getTime() === refetched.updatedAt.getTime() && updated.body === refetched.body,
        } as const;
      } catch (e: any) {
...
function deriveHints(args: { body?: string; appendBody?: string; addTags?: string[]; removeTags?: string[] }) {
  const hints: string[] = [];
  if (args.body && args.appendBody) hints.push("Provide only one of body or appendBody");
  if (args.appendBody === "false") hints.push("Do not send literal 'false' as appendBody");
  if (!args.body && !args.appendBody && !args.addTags && !args.removeTags) hints.push("Provide at least one field to modify (entity, tags, body, or appendBody)");
  if (args.addTags && args.removeTags && args.addTags.some(t => args.removeTags?.includes(t))) hints.push("Avoid adding and removing the same tag in one call");
