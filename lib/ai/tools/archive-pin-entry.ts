...
export const archivePinEntry = ({ session, chatId }: { session: AppSession; chatId: string }) =>
  tool({
...
`,
    inputSchema: z.object({
      slug: z.string().min(1).describe("Slug of the archive entry to pin (must already exist)"),
    ...
    execute: async ({ slug, chatId: override }) => {
      ...
      try {
        const res = await pinArchiveEntryToChat({ userId: session.user.id, chatId: targetChatId, slug });
