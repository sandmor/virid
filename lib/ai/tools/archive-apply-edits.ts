...
function applyEdits(body: string, edits: Edit[]) {
  const results: Array<{ index: number; mode: string; target: string; status: "applied" | "skipped"; reason?: string; }> = [];
  let current = body;
  edits.forEach((e, i) => {
    if (!e.target) {
      results.push({ index: i, mode: e.mode, target: e.target, status: "skipped", reason: "empty target" });
      return;
    }
    if ((e.mode === "replace" || e.mode === "insertAfter" || e.mode === "insertBefore") && !e.text) {
      results.push({ index: i, mode: e.mode, target: e.target, status: "skipped", reason: "missing text" });
      return;
    }
    ...
    if (e.mode === "remove" || e.mode === "replace") {
      ...
      if (occAll) {
        if (!current.includes(e.target)) {
          results.push({ index: i, mode: e.mode, target: e.target, status: "skipped", reason: "not found" });
          return;
        }
        ...
        current = current.slice(0, pos) + replacement + current.slice(pos + e.target.length);
        applied = 1;
      }
      ...
      if (occAll) {
        ...
        if (e.mode === "insertAfter") {
          current = parts.slice(0, -1).map(p => p + e.target + insertText).join("") + parts.at(-1);
        } else {
          current = parts[0] + parts.slice(1).map(p => insertText + e.target + p).join("");
        }
    ...
    results.push({ index: i, mode: e.mode, target: e.target, status: applied > 0 ? "applied" : "skipped", reason: applied === 0 ? "no occurrences" : undefined });
  });
...
export const archiveApplyEdits = ({ session }: { session: AppSession }) =>
  tool({
    description: "Apply surgical edits to an archive entry (anchor-based partial modifications) without rewriting the entire body. Does NOT record revision notes inside the body.",
    inputSchema: z.object({
      slug: z.string().min(1),
      edits: z.array(z.object({
        mode: z.enum(["replace", "insertAfter", "insertBefore", "remove"]).describe("Edit operation kind"),
        target: z.string().min(1).describe("Anchor substring to locate in current body"),
        text: z.string().optional().describe("Replacement or insertion text (omit for remove)"),
        occurrences: z.enum(["first", "all"]).optional().describe("Apply to first match or all matches (default first)"),
      })).min(1).max(25),
    }),
