...
export const archiveLinkEntries = ({ session }: { session: AppSession }) =>
  tool({
    description: "Create a semantic relationship link between two archive entries.",
    ...
    execute: async ({ sourceSlug, targetSlug, type, bidirectional }) => {
      ...
      try {
        const res = await linkArchiveEntries({ userId, sourceSlug, targetSlug, type, bidirectional });
