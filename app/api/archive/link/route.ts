...
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await linkArchiveEntries({ userId: session.user.id, sourceSlug, targetSlug, type, bidirectional });
    return Response.json(res);
  } catch {
    return new ChatSDKError("bad_request:api","Failed to link entries").toResponse();
  }
}
