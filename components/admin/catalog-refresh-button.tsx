...
export function CatalogRefreshButton() {
  ...
  async function refresh(force = true) {
    ...
    if (now - lastClickRef.current < 1500) {
      toast({ type: "error", description: "Please wait before refreshing again" });
      return;
    }
    lastClickRef.current = now;
    setLoading(true);
    try {
      window.dispatchEvent(new CustomEvent("catalog:refresh", { detail: { force } }));
  ...
  return (
    <Button type="button" variant="secondary" size="sm" onClick={() => refresh(true)} disabled={loading}>
